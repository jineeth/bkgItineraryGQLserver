...
app.post('/', (req, res) => {
    ...
    if (bookingCode !== '' && familyName !== '') {
        ...
        let response = bookingList.filter(item => item.bookingCode === bookingCode && item.passengers.lastName === familyName)[0];
        ...
        res.status(400).send({ 'message': 'Invalid Booking details' });
    }
